...
impl Process {
    ...
    pub fn list_fds(&self) -> Option<Vec<usize>> {
        let mut fd_res: Vec<usize> = Vec::new();
        let target_path = format!("/proc/{}/fd", self.pid);
        let entrys = fs::read_dir(target_path).ok()?;
        for entry in entrys {
            let fine_entry = entry.ok()?;
            let fine_file_name = fine_entry.file_name().into_string().ok()?;
            fd_res.push(fine_file_name.parse::<usize>().ok()?);
        }
