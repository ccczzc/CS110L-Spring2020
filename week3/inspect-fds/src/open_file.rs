...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let fd_file_path = fs::read_link(link_path).ok()?;
        let fd_file_name = OpenFile::path_to_name(fd_file_path.to_str()?);
        let fdinfo_file_path = format!("/proc/{pid}/fdinfo/{fd}");
        let fdinfo_file_content = fs::read_to_string(fdinfo_file_path).ok()?;
        let fd_cursor = OpenFile::parse_cursor(&fdinfo_file_content)?;
        let fd_access_mode = OpenFile::parse_access_mode(&fdinfo_file_content)?;
        Some(OpenFile::new(fd_file_name, fd_cursor, fd_access_mode))
    }
